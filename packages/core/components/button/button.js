...
const Button = {
  loading(el) {
    ...
    if ($el.length) {
      ...
      const text = elem.dataset.loadingText ? elem.dataset.loadingText : elem.innerText;
      const color = elem.dataset.loadingIconClass ? elem.dataset.loadingIconClass : 'color-white';
      const icon = `<span class="preloader ${color}"></span>`;
